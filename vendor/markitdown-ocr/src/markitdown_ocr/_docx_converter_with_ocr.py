...
from ._ocr_service import LLMVisionOCRService, _format_ocr_block
    ...
    def convert(
        ...
        **kwargs: Any,
        ...
        if ocr_service:
            ...
            #    Each entry in ocr_texts is ALREADY a full marked block
            #    (success *[Image OCR]... or error *[OCR Error]...), produced
            #    by _format_ocr_block, so we substitute verbatim.
            for i, raw_text in enumerate(ocr_texts):
                placeholder = _PLACEHOLDER.format(i)
                md = md.replace(placeholder, raw_text)
            ...
            for rel in doc.part.rels.values():
                if "image" in rel.target_ref.lower():
                    try:
                        ...
                        # Store the full marked block (success OR error). A
                        # failed OCR is preserved as an *[OCR Error]... block
                        # rather than being silently dropped.
                        block = _format_ocr_block(ocr_result)
                        if block is not None:
                            ocr_map[rel.rId] = block
